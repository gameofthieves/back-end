...
exports.parse = (data, user) => {
  let message = data.trim().toString();
  ...
  // if game is in session and player is not an active player, cannot talk
  if (server.all[user.currentServer].phase && server.all[user.currentServer].activePlayers.indexOf(user) === -1) {
    user.socket.write(`\tYou are not a currently active player in the game. You cannot speak or perform actions.\n\n`);
    return;
  }
  ...
  if (message.slice(0, 1) === '@') {
    ...
    case '@create':
      ...
      server.all[user.currentServer].players = server.all[user.currentServer].players.filter(el => el.user !== user.user);
      server.all[user.currentServer].players.map(usr => usr.socket.write(`\t${user.nick} has left the room ${user.currentServer}.\n\n`));
    ...
    case '@join':
      ...
      if (server.all[descriptor].closed) {
        user.socket.write(`\tRoom already full or game has already started.\n\n`);
        return;
      }
      if (server.all[user.currentServer].phase && server.all[user.currentServer].activePlayers.indexOf(user) > -1)
      {
        user.socket.write(`\tYou can't join another room while you are an active player in a current game.\n\n`);
        return;
      }
      server.all[user.currentServer].players = server.all[user.currentServer].players.filter(el => el.user !== user.user);
      server.all[user.currentServer].players.map(usr => usr.socket.write(`\t${user.nick} has left the room ${user.currentServer}.\n\n`));
      server.all[descriptor].players.push(user);
      server.all[descriptor].players.map(usr => usr.socket.write(`\t${user.nick} has joined the room ${descriptor}.\n\tThere are ${server.all[descriptor].players.length} players connected; ${7 - server.all[descriptor].players.length} more players needed.\n\n`));
      user.currentServer = descriptor;
    ...
    case '@roles':
      ...
      LOCKSMITH [town]: Can protect one player each night from being robbed. Use @action <playername> during night phase to protect. Cannot protect self.
      ...
      THIEF RECRUITER [thief]: Can recruit one town-aligned player per game, during the night phase, to join the thieves. Can save this ability to use on a different night. Use @action <playername> to recruit. Immune to being blocked.
      \n`);
      break;
    ...
    case '@players':
      if (!server.all[user.currentServer].phase) {
        user.socket.write(`\tUsers in room at ${user.currentServer}: \n\t${server.all[user.currentServer].players.map(el => el.nick).join(', ')}\n\n`);
        break;
      }
      user.socket.write(`\t ${server.all[user.currentServer].activePlayers.length} users still playing in the game at ${user.currentServer} room: \n\t${server.all[user.currentServer].activePlayers.map(el => el.nick).join(', ')}\n\n`);
      break;
    ...
    // Quits the game
    case '@quit':
    // If game is in progress, show msg that they are killed and their role, take them off currentplayers and activeplayers lists


      user.socket.write(`\tSee you later, ${user.nick}\n`);
      user.socket.end();
      break;
    ...
    case '@action': {
      ...
      case 'LOCKSMITH':
        ...
        user.socket.write(`\tYour action has been noted.\n\n`);
        break;
      case 'COP':
      ...
      case 'THIEF RECRUITER':
        ...
        break;
      default:
        user.socket.write(`\tYou cannot perform an action.\n\n`);
      }
      break;
    }
    ...
    if (server.all[user.currentServer].phase && user.mute) {
      user.socket.write(`\tYou were visited by the dentist last night and can't talk today. You can still perform actions.\n\n`);
      return;
    }
    // if in home server or day phase, all active players in the room can talk
    else if (user.currentServer === 'home' || server.all[user.currentServer].phase === 'day') {
      server.all[user.currentServer].players.filter(usr => usr.user !== user.user)
    ...
    // if night phase, only thieves can talk to each other
      ...
      if (user.alignment === 'town') {
        user.socket.write(`\tMessage not delivered: only thieves can talk during night phase.\n\n`);
        return;
      }
      server.all[user.currentServer].players.filter(usr => usr.alignment !== 'town')
        .map(usr => usr.socket.write(`${user.nick} [thieves]: ${message}\n\n`));
    }
  }
};