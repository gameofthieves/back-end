...
exports.parse = (data, user) => {
  ...
  console.log(user.currentServer, server.all[user.currentServer].phase);
 ...
 {bold @lastwords} - save player's last words to output to room 
    ...
    case '@join': {
      ...
      else server.all[descriptor].players.map(usr => usr.socket.write(`\n\t` + roomMsg(` ${user.nick} has joined the room ${descriptor}. \n`)));

      // deletes previous server if now empty
      if (temp !== 'home' && !server.all[temp].players.length) delete server.all[temp];
      break;
    }
...
`));
    ...
    // if no game in progress or during day phase, all active players in the room can talk
      ...
      user.socket.write((chalk`> {bold ${user.nick}}({italic self}): ${message}\n`));
    }
