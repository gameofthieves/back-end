...
exports.parse = (data, user) => {
  ...
  if (message.slice(0, 1) === '@') {
    ...
    case '@about':
      figlet(` About GoT  `, { font: 'slant' }, (err, text) => {
 ...
 ends when no thieves are left or when no town are left.
    ...
    case '@help':
      figlet(` Help Menu `, { font: 'slant' }, (err, text) => {
        return new Promise((resolve, reject) => {
          if (err) return reject(new Error('Figlet'));
          user.socket.write(`${text}`);
          return resolve();
        })
          .then(() => user.socket.write(chalk`
 ==================== {bold Game Commands} =====================
 ...
 {bold @lastwords} - save player's last words to output to room 
    ...
    case '@join':
      ...
      if (server.all[descriptor].players.length === 2) {
    ...
    case '@rooms':
      user.socket.write(`\n\t` + gameInfo(` Current rooms: ${Object.keys(server.all).join(', ')} \n`));
      break;
    
    // Lists all possible roles in play and a description of each role
    case '@roles':
      figlet(` Game Roles `, { font: 'slant' }, (err, text) => {
        return new Promise((resolve, reject) => {
          if (err) return reject(new Error('Figlet'));
          user.socket.write(`${text}`);
          return resolve();
...
`));
    ...
    case '@players':
      if (!server.all[user.currentServer].phase) {
        user.socket.write(`\n\t` + gameInfo(` Users in room {bold ${user.currentServer}} (${server.all[user.currentServer].players.length}): \n`) + `\t` + gameInfo(` ${server.all[user.currentServer].players.map(el => el.nick).join(', ')} \n`));
        break;
      }
    ...
    case '@vote':{
      ...
      if (!isValidName) {
        user.socket.write(`\n\t` + serverResponse(` Not a valid player. Use @player to see players in game. \n`));
        break;
      }
      server.all[user.currentServer].players.map(usr => usr.socket.write(chalk.bold(`\t ##VOTE: ${user.nick}: ${descriptor}.\n\t Use @votes to see current votes for the day. \n`)));
    ...
    case '@votes': {
      ...
      user.socket.write(gameInfo(`${str}`));
      break;
    }
    ...
    if (server.all[user.currentServer].phase && user.mute) {
      user.socket.write(`\n\t` + serverResponse(` You were visited by the dentist last night and can't talk today. You can still perform actions. \n`));
      return;
    }
    // if no game in progress or during day phase, all active players in the room can talk
      ...
      server.all[user.currentServer].players.filter(usr => usr.user !== user.user)
        .map(usr => usr.socket.write(chalk`> {bold ${user.nick}}: ${message}\n`));
    }
    // if night phase, only thieves can talk to each other
      ...
      server.all[user.currentServer].players.filter(usr => usr.alignment !== 'town')
        .map(usr => usr.socket.write(chalk`> {bold ${user.nick}} [{italic thieves}]: ${message}\n`));
    }
  }
};