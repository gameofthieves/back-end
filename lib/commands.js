...
exports.parse = (data, user) => {
  ...
  if (message.slice(0, 1) === '@') {
    ...
    case '@create':
      ...
      currentPlayers = currentPlayers.filter(el => el !== user);
      currentPlayers.map(user => user.socket.write(`\t${user.nick} has left the room ${user.currentServer}.\n\n`));
    ...
    case '@players':
      user.socket.write(`\t${currentPlayers.length} users connected in ${user.currentServer} room: \n\t${currentPlayers.map(el => el.nick).join(', ')}\n\n`);
      break;
    ...
    // if day phase
    currentPlayers.filter(c => c !== user)
