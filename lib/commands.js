...
exports.parse = (data, user) => {
  ...
  if (message.slice(0, 1) === '@') {
    ...
    // NOT AN @ command
    // if in home server, everyone in the room can talk
    if (user.currentServer === 'home') {
    ...
    // if game is in session and player is not an active player, cannot talk
      ...
      user.socket.write(`\tYou are not a currently active player in the game. You cannot speak.\n\n`);
    }
    // if day phase, all active players in the room can talk
    else if (server.all[user.currentServer].phase === 'day') {
      server.all[user.currentServer].players.filter(usr => usr !== user)
        .map(usr => usr.socket.write(`${user.nick}: ${message}\n\n`));
    }
