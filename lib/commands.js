...
exports.parse = (data, user) => {
  ...
  if (user.currentServer === 'login') {
    let path = `:${process.env.PORT }/api/v1`;
      ...
      superagent.get(`${path}/login`)
        ...
        .then(res => {
          if (res.status === 200) {
            ...
            // server.welcome(res.body._id);
          }
        })
        .catch(err => {
          console.log(err);
          user.socket.write(serverResponse(` Could not log in. `));
        });
      return;
    }
      ...
      superagent.post(`${path}/register`)
        ...
        .then(res => {
          if (res.status === 201) {
            ...
            user.socket.write(serverResponse(' Invalid registration. '));
          }
        })
        .catch(err => {
          console.log(err);
          user.socket.write(serverResponse(` Could not register. `));
        });
      return;
    }
    else {
