...
game.start = user => {
  ...
  let thisServer = user.currentServer;
...
`)));
  ...
  function phase() {
    ...
    if (server.all[user.currentServer].phase === 'day') {
      ...
      server.all[user.currentServer].interval = setInterval(phase, 60000); // 1 minute for day phase

      // tallies night actions and amends active player list
      nightActions(user, thisServer);
...
function tallyDayVotes(user, thisServer) {
  if (thisServer !== user.currentServer) {
    console.log('tally day votes');
    return;
  }
  let obj = {};
...
function nightActions(user, thisServer) {
  if (thisServer !== user.currentServer) {
