...
const gameInfo = chalk.whiteBright.bgCyan;
...
game.start = user => {
  ...
  const assignRoles = () => {
    return new Promise((resolve, reject) => {
      ...
      // error checking
      if (!allroles.length) return reject(new Error('No roles to assign.'));
  ...
  // Then Sends game start message with role to each player
  assignRoles()
    .then(() => {
      figlet(` Game Start `, (err, text) => {
        return new Promise((resolve, reject) => {
          if (err) return reject(new Error('Figlet'));
          server.all[user.currentServer].players.map(usr => usr.socket.write((gameInfo(`\n\n${text}\n`))));
          return resolve();
        })
          .then(() => {
          ...
          .then(() => server.all[user.currentServer].players.map(usr => usr.socket.write('\n')));
      });
    });
