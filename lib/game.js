...
`)));
  ...
  function phase() {
    console.log(server.all[user.currentServer].gameEnded, !server.all[user.currentServer].gameEnded);

    if (server.all[user.currentServer].gameEnded || server.all[user.currentServer].gameEnded === undefined) {
    ...
    if (server.all[user.currentServer].phase === 'day') {
      ...
      server.all[user.currentServer].interval = setInterval(phase, 15000); // 30s night phase
...
function tallyDayVotes(user) {
  let obj = {};
  console.log('dayvotes: ', user.currentServer);
