...
game.start = user => {
  ...
  game.phase = () => {
    ...
    if (align.indexOf('thief') === -1 || align.indexOf('town') === -1) {
      ...
      server.all['home'].players.push(user);
      user.socket.write(`\tThank you for playing! You will be redirected to the home room.\n`);
      server.all['home'].players.map(user => user.socket.write(`\t${user.nick} has joined the room home.\n\tThere are ${server.all['home'].players.length} players in this room.\n\n`));
    ...
    if (server.all[user.currentServer].phase === 'day') {
      ...
      if (server.all[user.currentServer].day) {
        ...
        // Last words
        let jailedUser = server.all[user.currentServer].players.filter(usr => usr.nick === name);
      ...
      game.interval = setInterval(game.phase, 30000); // 30sec night phase
    }
      ...
      game.interval = setInterval(game.phase, 60000); // one minute for day phase
    }
