...
// start game
game.start = user => {
  console.log(`Game started in room ${user.currentServer}`);
  ...
  server.all[user.currentServer].interval = null;
  let interval = null;
...
`)));
  ...
  // handling phasing
  server.all[user.currentServer].gameEnded = false;
  function phase() {

    try {
      if (!server.all[user.currentServer] || server.all[user.currentServer].gameEnded || server.all[user.currentServer].gameEnded === undefined) {
    ...
    if (server.all[user.currentServer].phase) clearInterval(server.all[user.currentServer].interval);
    ...
    if (server.all[user.currentServer].phase === 'day') {
      ...
      server.all[user.currentServer].interval = setInterval(phase, 60000); // 1 minute night phase
      interval = server.all[user.currentServer].interval;
      ...
      if (checkWinner(user)) {
        clearInterval(server.all[user.currentServer].interval);
        return;
      }
    }
      ...
      server.all[user.currentServer].interval = setInterval(phase, 90000); // 1 minute 30s for day phase
      interval = server.all[user.currentServer].interval;
...
function checkWinner(user) {
  ...
  if (align.indexOf('thief') === -1 || align.indexOf('town') === -1) {
    let winner = (align.indexOf('thief') === -1) ? 'town' : 'thief';
    server.all[user.currentServer].gameEnded = true;
...
`)));
    ...
    // gets players' records and updates it
    server.all[user.currentServer].players.forEach(usr => {
      let newGamesPlayed = Number(usr.gamesPlayed) + 1;
      let newGamesWon = Number(usr.gamesWon) + 1;
      if (usr.alignment === winner) {
        superagent.put(`${path}/profile/${usr.profileId}`)
          .send({
            gamesPlayed: newGamesPlayed.toString(),
            gamesWon: newGamesWon.toString(),
            percentWon: ((newGamesWon / newGamesPlayed) * 100).toFixed(2).toString(),
          })
        ...
        superagent.put(`${path}/profile/${usr.profileId}`)
          .send({
            ...
            percentWon: ((Number(usr.gamesWon) / newGamesPlayed) * 100).toFixed(2).toString(),
          })
          .catch(err => console.log(err.message));
      }
    });
    ...
    console.log(`Game ended in room ${tempServer}.`);
...
function nightActions(user) {
  ...
  server.all[user.currentServer].players.forEach(usr => {
    ...
    if (usr.actionUsed) {
      usr.socket.write(`\n\t` + serverResponse(` You have already used your night action. \n`));
      return;
    }

    else if (usr.role === 'JAILOR' && usr.targeting && !usr.converted) {
      usr.targeting.targeting = null;
    }

    else if (usr.role === 'DENTIST' && usr.targeting && !usr.converted && usr.targeted.indexOf('jailed') === -1) {
    ...
    else if (usr.role === 'COP' && usr.targeting && !usr.converted && usr.targeted.indexOf('jailed') === -1) {
      if (!usr.nightmsg) {
        usr.nightmsg = chalk`You investigated {bold ${usr.targeting.nick}} and their alignment is {bold ${usr.targeting.alignment}}.`;
      }
    }

    else if (usr.role === 'CREEPER' && usr.targeting && !usr.converted && usr.targeted.indexOf('jailed') === -1) {
    ...
    else if (usr.role === 'THIEF' && usr.targeting && usr.targeted.indexOf('jailed') === -1 && usr.targeting.targeted.indexOf('protected') === -1) {
  ...
  server.all[user.currentServer].players.forEach(usr => {
    ...
    usr.converted = false;
    usr.nightmsg = '';
    if (usr.role !== 'THIEF RECRUITER') usr.actionUsed = false;
  });
}
