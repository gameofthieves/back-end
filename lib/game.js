...
game.start = user => {
  ...
  game.phase = () => {
    ...
    if (align.indexOf('thief') === -1 || align.indexOf('town') === -1) {
      ...
      currentPlayers.map(usr => {
        usr.socket.write(`\tThank you for playing! You will be redirected to the home room.\n`);
        server.all['home'].players.push(usr);
        server.all[usr.currentServer].players = server.all[usr.currentServer].players.filter(el => el !== usr);
    ...
    if (server.all[user.currentServer].phase === 'day') {
      ...
      if (server.all[user.currentServer].day) {
        ...
        // Last words
        console.log('last words user: ', name);
        let jailedUser = server.all[user.currentServer].players.filter(usr => usr.nick === name)[0];
        console.log('jailed user: ', jailedUser.lastwords);
