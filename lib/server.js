...
server.on('connection', function (socket) {
  ...
  user.welcome = id => {
    ...
    server.all[user.currentServer].players.filter(usr => usr.nick !== user.nick).map(usr => usr.socket.write(`\n\t` + roomMsg(` ${user.nick} has joined the game \n`)));
...
`));
  ...
  socket.on('close', function () {
    server.all[user.currentServer].players = server.all[user.currentServer].players.filter(usr => usr.nick !== user.nick);
    ...
    // if game is in progress and user is still an active user
      ...
      server.all[user.currentServer].activePlayers = server.all[user.currentServer].activePlayers.filter(usr => usr.nick !== user.nick);
    }
